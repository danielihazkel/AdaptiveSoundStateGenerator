...
**Personalization** (`src/personalization/`): a Thompson-sampling multi-armed bandit learns which sound variation works best for you, per state. Arms are perturbation *recipes* (slower/faster beat, alternate noise color, softer binaural layer, a harmonic pad on/off, more bass, a warmer tone, a different ambience, a sense of space, …) applied to the state's default profile at serve time — 14–16 per state, each gated on being audible against that state's defaults. Rewards blend explicit 1–5 ratings (and the optional distraction / use-again answers) with implicit signals — completion fraction, repeated volume tweaks or pauses (the first of each is free), an explicitly skipped rating; replaying a session, or a preset saved from one, credits the arm that produced the sound, and even a session cut short by a crash leaves a small down-weighted signal. Evidence decays with a 60-session half-life so old ratings never pin the posterior; each arm's posterior spread comes from its own observed reward variance (noisy arms keep exploring, consistent ones converge faster, with a floor so exploration never fully stops), and every arm also keeps statistics per **serving context** (time of day × headphones/speakers), shrunk toward the state-level posterior — so "slower beat works at night" is learnable without fragmenting the data. The first 6 sessions per state serve the pure §8 defaults (cold start); afterwards the app experiments unless you flip the per-state "lock what works" toggle — and **every 8th session it quietly serves the untouched default again** as a held-out baseline, so the "Is personalization working?" panel on the insights screen can honestly compare your default sound against the personalized ones (PRD §18's core proof point) instead of leaning on early sessions from before the app knew you. Adding recipes bumps the candidate-set version and the posterior is rebuilt from the session records rather than reset. Completed sleep sessions are rated via a next-morning prompt on the next app open. Once a state has 5+ sessions, the "Your sound profile" screen shows what's working: most effective layers, every variation tried with its score and interval, what works best at which time of day, preferred beat range, noise color, volume, and typical duration.
...
**Biometrics** (`src/biometrics/`): optional and consent-gated. A Web Bluetooth heart-rate strap (standard GATT profile; Chrome/Edge only) feeds a baseline-relative trend into the adaptation loop for relax/sleep/meditation — and, when the strap reports beat-to-beat RR intervals (most chest straps do), an **HRV (RMSSD) trend** too: for calm states falling HRV counts as adverse exactly like a rising heart rate, and once enough sessions carry HRV data the insights screen reports how your HRV moves while you listen. With the sensor connected, sleep sessions can opt into **"Fade out once I'm asleep"**: a conservative detector (`sleepOnset.ts` — heart rate sustained and steady below the settling-in baseline) winds the session down early with the normal minute-long sleep fade, counts it as fully completed (falling asleep is the point), records the onset time, and shows a "fell asleep" badge in history; "+15 min" during the fade cancels it. Raw readings and RR intervals live only in memory — sessions keep just per-segment summary deltas. Append `?simhr` to the URL for a simulated sensor during development (`?simhr=sleep` simulates falling asleep).
...
- **Phase 9 — Program sound overrides + reverb, HRV + sleep-onset** ✅ per-phase absolute overrides (beat, carrier, noise colour, ambience type, pad richness, space) with gliding boundaries, parametrized worklet crossfades, the synthesized-IR reverb send with its `space-on` bandit arm (candidate set v4), HRV (RMSSD) trends from BLE RR intervals as a second adaptation signal, and the opt-in sleep-onset fade for sleep sessions
- **Phase 10 — Prove the flywheel** ✅ held-out baseline serves, the "Is personalization working?" insights panel, pause tracking + recovered-session credit as implicit signals, and the empirical-variance posterior
